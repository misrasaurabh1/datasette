...
class Response:
    def __init__(self, body=None, status=200, headers=None, content_type="text/plain"):
        # headers param is assumed already a dict or None, no need for "or {}" at every __init__
        self.body = body
        self.status = status
        self.headers = headers if headers is not None else {}
        self._set_cookie_headers = []  # always an empty list at construction
        self.content_type = content_type
    ...
    def text(cls, body, status=200, headers=None):
        # Fast path for string, to avoid unnecessary str() call
        if type(body) is str:
            body_str = body
        else:
            body_str = str(body)
        # Use positional arguments for faster call dispatch
        return cls(body_str, status, headers, "text/plain; charset=utf-8")
